<template>
	<div class="address-box">
		...
		<div class="add-item name-item">
			<input type="text" v-model="realName.value" placeholder="真实姓名">
		</div>
		<div class="add-item">
			<input type="tel" v-model="mobile.value" placeholder="手机号码">
		</div>
		<div class="add-item">
		...
		<div class="address-details-item">
			<textarea v-model="detailsAddress.value" placeholder="详细信息"></textarea>
		</div>
		<div class="default-address">
			<a href="javascript:;" class="default" :class="{active : flag==1}" v-on:click="isDefault()"><i></i><span>设为默认地址</span></a>
		</div>
	</div>
</template>
	...
	export default{
		...
		data(){
			return{
				...
				mobile:{value:'',success:false,errorInfo:'请输入手机号'},
		...
		methods:{
           ...
           checkMobile(){
           	if(/^0?1[3|4|5|7|8]\d{8}$/.test($.trim(this.mobile.value))){
         		...
         		if(this.mobile.success){
         			...
         			if(this.province.success){
         				...
         				if(this.detailsAddress.success){
         					this.axios.post('https://www.easy-mock.com/mock/5a0bad8922eff44db04955c0/example_1510714761014/insertAddr',{
         						area_name:$('#pro').val()+''+$('#city').val()+''+$('#area').val(),
         						address:this.detailsAddress.value,
         						person_name:this.realName.value,
         						serv_num:this.mobile.value,
         						car_Id:this.cardId.value,
         						flag:this.flag
         					}).then(function(data){
