<template>
	<div class="address-box">
		...
		<div class="has-address" v-if="addressList.length>0">
			...
			<router-link class="add-new-address" to="addAddress" v-on:click="goAdd()">添加新地址</router-link>
		</div>
...
import toast from '../../components/toast.vue';
	export default{
		...
		methods:{
			chooseAddress(addId){
               if(this.$route.query.formAdderss==1){
               	commonMethods.setCookie('PALACE_ORDER_ADDRESS',addId);
               	this.$route.push('surePay');
               }
			},
			getAddressList(){
				var oThis = this;
				$.ajax({
					type:'post',
					url:'https://www.easy-mock.com/mock/5a0bad8922eff44db04955c0/example_1510714761014/address',
					dataType:'json',
					success:function(data){
						
						if(data.data.message){
							oThis.addressList = data.data.adr;
							var a = oThis.addressList;
							function length(o) {
								var count = 0;
								for(var i in o){
									count ++;
			...
			defaultAdress(addrId,flag,index){
			 ...
			 deleteAddress(addrId) {
			 	...
			 	$.ajax({
			 		type:"post",
			 		url:"https://www.easy-mock.com/mock/5a0bad8922eff44db04955c0/example_1510714761014/deleteAddress",
			 		data:{
			 			userAddrId : addrId,
			 		},
			 		dataType:'json',
